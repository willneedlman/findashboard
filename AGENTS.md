# Finance Dashboard — Codex Project Instructions
...
*User-global* (`~/.Codex/settings.json` — this machine, every project; not in the repo):
...
*Project-scoped* (`.Codex/settings.json` — travels with the repo):
- `Stop` → frontend typecheck (`tsc --noEmit`) when `frontend/src/**/*.ts(x)` changed; advisory, no-ops otherwise

See `.Codex/README.md` for the full setup inventory: the `/deploy` and `/new-tool` skills, the `frontend-test-writer` subagent, the `.mcp.json` servers (context7, github), and the installed plugins.
