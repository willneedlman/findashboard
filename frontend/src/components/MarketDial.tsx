...
export default function MarketDial({ showLegend = false }: { showLegend?: boolean }) {
  ...
  const dial = (
    <svg viewBox="0 0 600 600" style={{ display: 'block', width: '100%', height: '100%' }}>
      {RINGS.map((m, i) => {
        const r = 128 + i * 12.5
        return <circle key={`t${m.id}`} cx={CX} cy={CY} r={r} fill="none" stroke={T.track} strokeWidth={9} />
      })}
      {RINGS.map((m, i) => {
        const r = 128 + i * 12.5
        return utcArcs(m, now).map((a, j) => (
          <path key={`${m.id}-${j}`} d={arcPath(r, a.t0, a.t1)} fill="none"
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 220px', gap: 28, alignItems: 'center', height: '100%', padding: 8 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        {RINGS.map((m, i) => {
          ...
          return (
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 10px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
              ...
              <span style={{ fontFamily: T.sans, fontSize: 12, color: T.muted, flex: 1, minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{m.name}</span>
