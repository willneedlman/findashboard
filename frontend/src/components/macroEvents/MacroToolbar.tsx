...
function Segmented<V extends string>({ options, value, onPick, danger }: {
  ...
  return (
    <div style={{ ...box, padding: 3, gap: 2, maxWidth: '100%', overflowX: 'auto' }}>
      {options.map(o => {
...
export default function MacroToolbar({ filters, onChange, count }: {
  ...
  return (
    <div style={{ display: isMobile ? 'grid' : 'flex', gridTemplateColumns: isMobile ? '1fr' : undefined, gap: 10, alignItems: 'center', margin: '16px 0' }}>
      ...
      <div style={{ ...box, padding: '7px 12px', gap: 8, minWidth: 0, overflowX: 'auto' }}>
        {isMobile ? <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', width: '100%', gap: 12 }}>
          <label style={{ minWidth: 0 }}><span style={{ display: 'block', marginBottom: 4, fontFamily: T.label, fontSize: 8, fontWeight: 700, letterSpacing: '0.1em', color: T.muted }}>FROM</span><input type="date" value={filters.from} onChange={e => onChange({ ...filters, from: e.target.value })} style={{ ...dateInput, width: '100%' }} aria-label="From date" /></label>
          <label style={{ minWidth: 0 }}><span style={{ display: 'block', marginBottom: 4, fontFamily: T.label, fontSize: 8, fontWeight: 700, letterSpacing: '0.1em', color: T.muted }}>TO</span><input type="date" value={filters.to} onChange={e => onChange({ ...filters, to: e.target.value })} style={{ ...dateInput, width: '100%' }} aria-label="To date" /></label>
        </div> : <>
