      ...
      {!fundamentals ? loading
        ...
        <main style={PANEL}>
          ...
          {step === 4 && <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 18, alignItems: 'center', padding: '18px 19px', borderTop: `1px solid ${T.border}`, background: T.goldTint(4) }}>
              ...
              <div style={{ textAlign: 'right' }}><div style={LABEL}>DCF value / share</div><div style={{ fontFamily: T.mono, fontSize: 27, fontWeight: 810, letterSpacing: 'var(--theme-num-tracking, normal)', color: T.gold }}>{fmtMoney(analysis?.dcf.value_per_share)}</div></div>
            </div>
            <div className="mv-three-metrics" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', borderTop: `1px solid ${T.border}` }}>
              {[
        ...
        <aside className="mv-rail" style={{ ...PANEL, position: 'sticky', top: 8 }}>
          <div style={{ padding: '18px 18px 15px', borderBottom: `1px solid ${T.border}` }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginTop: 4 }}><span style={{ fontFamily: T.mono, fontSize: 33, fontWeight: 820, letterSpacing: 'var(--theme-num-tracking, normal)', color: T.gold }}>{fmtMoney(connectedValue)}</span><span style={{ fontFamily: T.mono, fontSize: 13, fontWeight: 770, color: (upside ?? 0) >= 0 ? T.pos : T.neg }}>{signedPct(upside)}</span></div>
