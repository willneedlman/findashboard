...
import { TrendingUp, Download, Plus, X, BarChart2 } from 'lucide-react'
      ...
      <tbody>
        {rows.map((r, i) => (
          <tr key={i} style={{ borderBottom: `1px solid ${C.border}22` }}>
            ...
            <td style={{ padding: '6px 10px', color: C.text }}>{r.se?.toFixed(6) ?? '—'}</td>
            <td style={{ padding: '6px 10px', color: r.t && Math.abs(r.t) > 2 ? C.green : C.text }}>
              {r.t?.toFixed(3) ?? '—'}
            </td>
            <td style={{ padding: '6px 10px' }}>{r.p != null ? <PValBadge p={r.p} /> : '—'}</td>
          </tr>
        ))}
      </tbody>
    </table>
  ...
  return (
    <div style={{ padding: '24px 28px', color: C.text, fontFamily: 'var(--theme-mono)', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
        ...
        <span style={{ fontSize: 18, fontWeight: 700, color: C.gold, letterSpacing: 1 }}>REGRESSION ANALYSIS</span>
      </div>
      ...
      {r && (
        <>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 20 }}>
            ...
            <StatCard label="F-Statistic" value={r.f_statistic?.toFixed(2) ?? '—'} />
