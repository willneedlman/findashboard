...
function ConciseAnalysis({ data }: { data: AnalysisResult }) {
  ...
  return <div className="portfolio-analysis-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gridTemplateRows: 'minmax(212px, 1.06fr) minmax(200px, 1fr) minmax(200px, 1fr)', gap: 10, flex: 1, minHeight: 0 }}>
    <Panel label="Sector allocation" meta="Current value · select a sector" style={{ gridColumn: '1 / -1', minHeight: 0, overflow: 'hidden' }}>
      <div className="portfolio-sector-layout" onPointerDownCapture={capturePopoverAnchor} style={{ height: '100%', display: 'grid', gridTemplateColumns: 'minmax(280px, .72fr) minmax(440px, 1.28fr)', alignItems: 'center', paddingTop: 4, overflow: 'hidden' }}>
    ...
    <Panel label="Monte Carlo range" meta={`${modelLabel(data.simulationSettings.model)} · ${data.simulationSettings.simulations} paths · ${data.simulationSettings.leverage.toFixed(1)}x · select a horizon`} style={{ gridColumn: '1 / -1', minHeight: 0, overflow: 'hidden' }}>
