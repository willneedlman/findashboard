...
import { fmtTailReturn, isTailLoss } from '../lib/format'
...
export default function PortfolioAnalysis() {
  ...
  return (
    <PageWrapper title="Portfolio Analysis">
      <div className="portfolio-analysis-page" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {book && <AnalysisHeader
          book={book}
          books={books}
          pending={m.isPending}
  ...
  useReportCapture(() => {
    const pieces: ClipDraft[] = [
      kpiClip('Portfolio Analysis', `${data.bookName} · Portfolio verdict`, [
        ...
        { label: 'Monte Carlo VaR 95', value: mc ? fmtTailReturn(mc.var_95) : 'Unavailable' },
        { label: 'Monte Carlo CVaR 95', value: mc ? fmtTailReturn(mc.cvar_95) : 'Unavailable' },
...
function ConciseAnalysis({ data }: { data: AnalysisResult }) {
  ...
  return <div className="portfolio-analysis-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gridTemplateRows: 'minmax(190px, 1fr) minmax(190px, 1fr) minmax(0, auto)', gap: 10, flex: 1, minHeight: 0 }}>
    <Panel label="Sector allocation" meta="Current value" style={{ minHeight: 0, overflow: 'hidden' }}>
      <div className="portfolio-sector-layout" onPointerDownCapture={capturePopoverAnchor} style={{ height: '100%', display: 'grid', gridTemplateColumns: 'minmax(130px, 1fr) minmax(150px, 1fr)', alignItems: 'center', paddingTop: 4, overflow: 'hidden' }}>
        <div className="portfolio-sector-chart" style={{ minWidth: 0, height: '100%', minHeight: 0, cursor: 'pointer', outline: 'none' }}><ResponsiveContainer width="100%" height="100%"><PieChart style={{ outline: 'none' }}><Pie isAnimationActive={false} data={sectorData} dataKey="weight" nameKey="sector" innerRadius="54%" outerRadius="88%" paddingAngle={1} stroke={T.surface} style={{ outline: 'none' }} onClick={(entry: any) => { const slice = sectorData.find(item => item.sector === (entry?.sector ?? entry?.payload?.sector)); if (slice) setSelection({ kind: 'sector', sector: slice.sector, memberSectors: slice.memberSectors }) }}>{sectorData.map((s, i) => <Cell key={s.sector} fill={sectorColors[i % sectorColors.length]} cursor="pointer" style={{ outline: 'none' }} />)}</Pie><Tooltip contentStyle={tipStyle} formatter={(value: number) => `${Number(value).toFixed(1)}%`} /></PieChart></ResponsiveContainer></div>
        <div className="portfolio-sector-legend" style={{ display: 'grid', gridTemplateColumns: sectorData.length > 6 ? 'repeat(2, minmax(0, 210px))' : 'minmax(0, 250px)', justifyContent: 'start', alignContent: 'center', gap: 2, minWidth: 0, width: '100%', maxWidth: '100%', height: '100%', padding: '2px 8px 2px 4px', overflowX: 'hidden', overflowY: 'auto' }}>{sectorData.map((s, i) => <button type="button" aria-label={`Inspect ${s.sector}, ${s.weight.toFixed(1)} percent`} aria-pressed={selection?.kind === 'sector' && selection.sector === s.sector} className="portfolio-inspectable portfolio-sector-row" onClick={() => setSelection({ kind: 'sector', sector: s.sector, memberSectors: s.memberSectors })} key={s.sector} style={{ appearance: 'none', border: '1px solid transparent', background: selection?.kind === 'sector' && selection.sector === s.sector ? T.hover : 'transparent', color: 'inherit', display: 'grid', gridTemplateColumns: '8px minmax(0, 1fr) 48px', gap: 8, alignItems: 'center', minWidth: 0, width: '100%', maxWidth: '100%', padding: '6px 7px', fontFamily: MONO, fontSize: 9.5, textAlign: 'left', cursor: 'pointer' }}><span style={{ width: 7, height: 7, background: sectorColors[i % sectorColors.length] }} /><span style={{ color: selection?.kind === 'sector' && selection.sector === s.sector ? T.text : T.muted, minWidth: 0, lineHeight: 1.25, overflowWrap: 'anywhere' }}>{s.sector}</span><span style={{ color: T.text, textAlign: 'right', whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>{s.weight.toFixed(1)}%</span></button>)}</div>
      </div>
      {selection?.kind === 'sector' && <DetailInspector selection={selection} data={data} anchor={popoverAnchor} onClose={() => setSelection(null)} />}
    </Panel>

    <Panel label="Return path" meta={`Growth of $100 vs ${BENCHMARK} · select a date`} style={{ minHeight: 0, overflow: 'hidden' }}>
      <div className="portfolio-chart-hit" onPointerDownCapture={capturePopoverAnchor} style={{ height: '100%' }}><ResponsiveContainer width="100%" height="100%"><ComposedChart data={data.backtest.cumulative} margin={{ top: 26, right: 14, bottom: 5, left: 0 }} onClick={(state: any) => { const point = chartPoint(state); if (point) setSelection({ kind: 'return', point }) }}><CartesianGrid stroke={T.borderFaint} vertical={false} /><XAxis dataKey="date" tick={{ fill: T.muted, fontSize: 9 }} minTickGap={70} /><YAxis tick={{ fill: T.muted, fontSize: 9 }} width={42} /><Tooltip contentStyle={tipStyle} /><ReferenceLine y={100} stroke={T.border} /><Line isAnimationActive={false} dataKey="portfolio" name={data.bookName} stroke={T.gold} strokeWidth={2} dot={false} activeDot={{ r: 4 }} /><Line isAnimationActive={false} dataKey="benchmark" name={BENCHMARK} stroke={T.blue} strokeWidth={1.2} dot={false} activeDot={{ r: 3 }} /></ComposedChart></ResponsiveContainer></div>
      {selection?.kind === 'return' && <DetailInspector selection={selection} data={data} anchor={popoverAnchor} onClose={() => setSelection(null)} />}
    </Panel>

    <Panel label="Downside path" meta="Peak-to-trough · select a date" style={{ minHeight: 0, overflow: 'hidden' }}>
      <div className="portfolio-chart-hit" onPointerDownCapture={capturePopoverAnchor} style={{ height: '100%' }}><ResponsiveContainer width="100%" height="100%"><AreaChart data={drawdowns} margin={{ top: 26, right: 14, bottom: 5, left: 0 }} onClick={(state: any) => { const point = chartPoint(state); if (point) setSelection({ kind: 'drawdown', point }) }}><CartesianGrid stroke={T.borderFaint} vertical={false} /><XAxis dataKey="date" tick={{ fill: T.muted, fontSize: 9 }} minTickGap={70} /><YAxis tick={{ fill: T.muted, fontSize: 9 }} width={42} /><Tooltip contentStyle={tipStyle} formatter={(value: number) => `${Number(value).toFixed(1)}%`} /><ReferenceLine y={0} stroke={T.border} /><Area isAnimationActive={false} dataKey="drawdown" stroke={T.neg} fill={mix(T.neg, 18)} activeDot={{ r: 4 }} /></AreaChart></ResponsiveContainer></div>
      {selection?.kind === 'drawdown' && <DetailInspector selection={selection} data={data} anchor={popoverAnchor} onClose={() => setSelection(null)} />}
    </Panel>

    <Panel label="Monte Carlo range" meta={`${modelLabel(data.simulationSettings.model)} · ${data.simulationSettings.simulations} paths · ${data.simulationSettings.leverage.toFixed(1)}x · select a horizon`} style={{ minHeight: 0, overflow: 'hidden' }}>
      {data.monteCarlo && bands.length ? <div className="portfolio-monte-layout" onPointerDownCapture={capturePopoverAnchor} style={{ height: '100%', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 118px', gap: 10 }}>
        <div className="portfolio-chart-hit" style={{ minWidth: 0, height: '100%' }}><ResponsiveContainer width="100%" height="100%"><ComposedChart data={bands} margin={{ top: 26, right: 5, bottom: 14, left: 0 }} onClick={(state: any) => { const point = chartPoint(state); if (point) setSelection({ kind: 'monte', point }) }}><CartesianGrid stroke={T.borderFaint} vertical={false} /><XAxis dataKey="day" tick={{ fill: T.muted, fontSize: 9 }} /><YAxis tick={{ fill: T.muted, fontSize: 9 }} width={44} /><Tooltip contentStyle={tipStyle} formatter={(value: number) => Number(value).toFixed(1)} /><ReferenceLine y={100} stroke={T.border} /><Area isAnimationActive={false} dataKey="p95" stroke="none" fill={mix(T.blue, 8)} /><Area isAnimationActive={false} dataKey="p75" stroke="none" fill={mix(T.blue, 14)} /><Area isAnimationActive={false} dataKey="p25" stroke="none" fill={T.surface} /><Area isAnimationActive={false} dataKey="p5" stroke="none" fill={mix(T.neg, 12)} /><Line isAnimationActive={false} dataKey="p50" name="Median" stroke={T.gold} strokeWidth={2} dot={false} activeDot={{ r: 4 }} /></ComposedChart></ResponsiveContainer></div>
        <div className="portfolio-monte-outcomes" style={{ padding: '2px 0 4px', display: 'flex', flexDirection: 'column', justifyContent: 'space-evenly', gap: 4, minHeight: 0, overflowY: 'auto' }}><Outcome label="Upside (95th)" value={terminal ? fmtPct((terminal.p95 - 1) * 100, 0) : '—'} color={T.pos} onClick={() => setSelection({ kind: 'monte', point: bands[bands.length - 1] })} /><Outcome label="Median" value={terminal ? fmtPct((terminal.p50 - 1) * 100, 0) : '—'} color={T.gold} onClick={() => setSelection({ kind: 'monte', point: bands[bands.length - 1] })} /><Outcome label="Downside (5th)" value={terminal ? fmtPct((terminal.p5 - 1) * 100, 0) : '—'} color={T.neg} onClick={() => setSelection({ kind: 'monte', point: bands[bands.length - 1] })} /><Outcome label="Worst 5% average" value={fmtTailReturn(data.monteCarlo.cvar_95, 0)} color={isTailLoss(data.monteCarlo.cvar_95) ? T.neg : T.pos} onClick={() => setSelection({ kind: 'monte', point: bands[bands.length - 1] })} /></div>
    ...
    <Panel label="Downtrend watch" meta="Select a holding for detail" style={{ gridColumn: '1 / -1', minHeight: 0, maxHeight: 208, overflowY: 'auto', padding: '34px 14px 10px' }}>
...
function Outcome({ label, value, color, onClick }: { label: string; value: string; color: string; onClick?: () => void }) {
  return <button type="button" className="portfolio-inspectable" onClick={onClick} style={{ appearance: 'none', border: 0, background: 'transparent', padding: '1px 4px', textAlign: 'left', cursor: onClick ? 'pointer' : 'default' }}><div style={{ color: T.muted, fontFamily: SANS, fontSize: 8, letterSpacing: '.06em', textTransform: 'uppercase', lineHeight: 1.2 }}>{label}</div><div style={{ color, fontFamily: MONO, fontSize: 14, fontWeight: 800, marginTop: 1, lineHeight: 1.15 }}>{value}</div></button>
}
...
function Overview({ data, benchmark }: { data: AnalysisResult; benchmark: string }) {
  ...
  return <div className="portfolio-analysis-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.65fr) minmax(300px, .8fr)', gap: 10 }}>
    ...
    <Panel label="Capital weight vs risk contribution" meta="Variance contribution" style={{ gridColumn: '1 / -1', minHeight: 300 }}><ResponsiveContainer width="100%" height={270}><BarChart data={riskRows} margin={{ top: 42, right: 18, left: 4, bottom: 4 }}><CartesianGrid stroke={T.borderFaint} vertical={false} /><XAxis dataKey="ticker" tick={{ fill: T.text, fontSize: 10 }} /><YAxis tick={{ fill: T.muted, fontSize: 9 }} /><Tooltip contentStyle={tipStyle} /><Bar isAnimationActive={false} dataKey="weight" name="Capital weight %" fill={T.blue} /><Bar isAnimationActive={false} dataKey="riskContribution" name="Risk contribution %" fill={T.gold} /></BarChart></ResponsiveContainer></Panel>
  </div>
}
...
function Performance({ data, benchmark }: { data: AnalysisResult; benchmark: string }) {
  ...
  return <div className="portfolio-analysis-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 10 }}>
    <Panel label="Cumulative performance" meta={`Portfolio vs ${benchmark}`} style={{ height: 340 }}><ResponsiveContainer width="100%" height="100%"><ComposedChart data={data.backtest.cumulative} margin={{ top: 40, right: 14, left: 0, bottom: 6 }}><CartesianGrid stroke={T.borderFaint} vertical={false}/><XAxis dataKey="date" tick={{ fill: T.muted, fontSize: 9 }} minTickGap={60}/><YAxis tick={{ fill: T.muted, fontSize: 9 }} width={44}/><Tooltip contentStyle={tipStyle}/><Line isAnimationActive={false} dataKey="portfolio" stroke={T.gold} dot={false} strokeWidth={2}/><Line isAnimationActive={false} dataKey="benchmark" stroke={T.blue} dot={false}/></ComposedChart></ResponsiveContainer></Panel>
    <Panel label="Drawdown" meta="Peak-to-trough" style={{ height: 340 }}><ResponsiveContainer width="100%" height="100%"><AreaChart data={dd} margin={{ top: 40, right: 14, left: 0, bottom: 6 }}><CartesianGrid stroke={T.borderFaint} vertical={false}/><XAxis dataKey="date" tick={{ fill: T.muted, fontSize: 9 }} minTickGap={60}/><YAxis tick={{ fill: T.muted, fontSize: 9 }} width={44}/><Tooltip contentStyle={tipStyle}/><Area isAnimationActive={false} dataKey="drawdown" stroke={T.neg} fill={mix(T.neg, 20)} /></AreaChart></ResponsiveContainer></Panel>
    <Panel label="Rolling beta" meta="60 trading days" style={{ height: 290, gridColumn: '1 / -1' }}><ResponsiveContainer width="100%" height="100%"><AreaChart data={beta} margin={{ top: 40, right: 14, left: 0, bottom: 6 }}><CartesianGrid stroke={T.borderFaint} vertical={false}/><XAxis dataKey="date" tick={{ fill: T.muted, fontSize: 9 }} minTickGap={80}/><YAxis tick={{ fill: T.muted, fontSize: 9 }} width={44}/><Tooltip contentStyle={tipStyle}/><ReferenceLine y={1} stroke={T.gold}/><Area isAnimationActive={false} dataKey="value" stroke={T.blue} fill={mix(T.blue, 14)} /></AreaChart></ResponsiveContainer></Panel>
  </div>
}
...
function ComparisonTable({ optimizer }: { optimizer: OptimizerData }) {
  ...
  return <div style={{ overflowX: 'auto' }}><table style={tableStyle}><thead><tr>{['Portfolio', 'Expected return', 'Volatility', 'Sharpe', 'VaR 95', 'CVaR 95'].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr></thead><tbody>{rows.map(([name, p]) => p && <tr key={name}><td style={{ ...tdStyle, color: name === 'Current' ? T.gold : T.text }}>{name}</td><td style={tdNum}>{fmtPct(p.return)}</td><td style={tdNum}>{p.vol.toFixed(1)}%</td><td style={tdNum}>{p.sharpe.toFixed(2)}</td><td style={tdNum}>{fmtTailReturn(p.var_95)}</td><td style={tdNum}>{fmtTailReturn(p.cvar_95)}</td></tr>)}</tbody></table></div>
}
