    ...
    <SidebarLayout sidebarWidth={0} sidebarTitle="" sidebar={<div style={{ display: 'none', padding: 14, flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ width: '100%', maxWidth: 1320, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 12, minHeight: 'calc(100vh - 220px)' }}>
        <div style={{ background: SURFACE, border: `1px solid ${BORDER}` }}>
          ...
          <div style={{ display: 'flex', height: 4, background: 'rgba(255,255,255,.05)' }}>{tickers.map((t, i) => <div key={i} style={{ flex: Math.max(weights[t] || 1, 1), background: i % 2 ? `color-mix(in srgb, ${GOLD} 70%, #000)` : GOLD }} />)}</div>
          {controlsCollapsed ? <div style={{ padding: '11px 16px', color: SEC, fontFamily: MONO, fontSize: 10 }}>{tickers.join(' · ')} · {lookback}Y · {constraintMode.replace('_', ' ')}</div> : <div style={{ display: 'flex', flexWrap: 'wrap' }}>
            <div style={{ flex: '1.4 1 420px', minWidth: 0, padding: '14px 16px', borderRight: `1px solid ${BORDER}` }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(150px, 100%), 1fr))', gap: 8 }}>
                {tickers.map((t, i) => <div key={i} style={{ background: 'color-mix(in srgb, var(--theme-surface, #0d1826) 100%, #000 8%)', border: `1px solid ${BORDER}`, padding: '8px 9px', boxShadow: '0 1px 5px rgba(0,0,0,.35)' }}>
