    ...
    <div className="ft-panel" style={{ overflow: 'hidden' }}>
      ...
      <div style={{ overflowX: 'auto' }}>
        <div className="scm-map-grid" style={{ position: 'relative', display: 'grid', gridTemplateColumns: 'minmax(240px, 1fr) minmax(230px, 0.72fr) minmax(240px, 1fr)', gap: 42, minWidth: 900, padding: '26px 30px 30px', minHeight: 560 }}>
          ...
          <section style={{ zIndex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-around', flex: 1, gap: 10 }}>{mapped.sourcing.length ? mapped.sourcing.map(p => <CompanyNode key={`${p.name}-${p.verified?.category ?? p.score}`} peer={p} side="sourcing" color={nodeColor(p)} sortMode={sortMode} dimmed={!!focus && focus !== p} selected={selected === p} onHover={setHovered} onSelect={setSelected} />) : <Unavailable label="No disclosed supplier or sourcing relationships for this issuer" />}</div>
            {layout.sourcing.length > MAX_MAP_NODES && <div style={{ height: 38, marginTop: 8 }}><button onClick={() => setExpandedSide(expandedSide === 'sourcing' ? null : 'sourcing')} style={{ width: '100%', padding: '8px 10px', background: 'transparent', border: `1px dashed ${T.border}`, color: GOLD, fontFamily: T.mono, fontSize: 9.5, fontWeight: 800, cursor: 'pointer' }}>{expandedSide === 'sourcing' ? 'HIDE FULL LIST' : `VIEW ${layout.sourcing.length - MAX_MAP_NODES} MORE`}</button></div>}
          </section>
          ...
          <section style={{ zIndex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-around', flex: 1, gap: 10 }}>{mapped.markets.length ? mapped.markets.map(p => <CompanyNode key={`${p.name}-${p.verified?.category ?? p.end_market?.fiscal_year ?? p.score}`} peer={p} side="markets" color={nodeColor(p)} sortMode={sortMode} dimmed={!!focus && focus !== p} selected={selected === p} onHover={setHovered} onSelect={setSelected} />) : <Unavailable label="No reported buyers or end-market revenue breakdown for this issuer" />}</div>
            {layout.markets.length > MAX_MAP_NODES && <div style={{ height: 38, marginTop: 8 }}><button onClick={() => setExpandedSide(expandedSide === 'markets' ? null : 'markets')} style={{ width: '100%', padding: '8px 10px', background: 'transparent', border: `1px dashed ${T.border}`, color: BLUE, fontFamily: T.mono, fontSize: 9.5, fontWeight: 800, cursor: 'pointer' }}>{expandedSide === 'markets' ? 'HIDE FULL LIST' : `VIEW ${layout.markets.length - MAX_MAP_NODES} MORE`}</button></div>}
          </section>
        </div>
      </div>
