---
source_file: "backend/routers/corporate.py"
type: "code"
community: "Backend"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Backend
---

# corporate.py

## Connections
- [[fastapi]] - `imports` [EXTRACTED]
- [[cache]] - `imports` [EXTRACTED]
- [[backend_routers_corporate_py_company_index]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_pretty_company]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_norm_company]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_yahoo_search]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_company_search]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_get_peers_for_ticker]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_safe_float]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_extract_news]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_extract_earnings_date]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_consensus_label]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_get_corporate_hub]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_get_corporate_hub_short]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_get_corporate_hub_insider]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_get_corporate_hub_analyst]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_synthetic_rating]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_leverage_rating]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_altman_rating]] - `contains` [EXTRACTED]
- [[backend_routers_corporate_py_ttm]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Backend