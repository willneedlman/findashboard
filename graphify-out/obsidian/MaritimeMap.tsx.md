---
source_file: "frontend/src/pages/MaritimeMap.tsx"
type: "code"
community: "Frontend"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Frontend
---

# MaritimeMap.tsx

## Connections
- [[frontend_src_pages_maritimemap_tsx_buildcolors]] - `contains` [EXTRACTED]
- [[frontend_src_pages_maritimemap_tsx_sizefix]] - `contains` [EXTRACTED]
- [[frontend_src_pages_maritimemap_tsx_vectorbasemap]] - `contains` [EXTRACTED]
- [[frontend_src_pages_maritimemap_tsx_focuscontroller]] - `contains` [EXTRACTED]
- [[frontend_src_pages_maritimemap_tsx_viewportwatcher]] - `contains` [EXTRACTED]
- [[frontend_src_pages_maritimemap_tsx_mapclickcatcher]] - `contains` [EXTRACTED]
- [[frontend_src_pages_maritimemap_tsx_buildlegend]] - `contains` [EXTRACTED]
- [[frontend_src_pages_maritimemap_tsx_sparkline]] - `contains` [EXTRACTED]
- [[frontend_src_pages_maritimemap_tsx_maritimemapcontent]] - `contains` [EXTRACTED]
- [[frontend_src_pages_maritimemap_tsx_statrow]] - `contains` [EXTRACTED]
- [[frontend_src_pages_maritimemap_tsx_nowcastblock]] - `contains` [EXTRACTED]
- [[frontend_src_pages_maritimemap_tsx_historypanel]] - `contains` [EXTRACTED]
- [[react]] - `imports` [EXTRACTED]
- [[tanstack_react_query]] - `imports` [EXTRACTED]
- [[react_router_dom]] - `imports` [EXTRACTED]
- [[axios]] - `imports` [EXTRACTED]
- [[react_leaflet]] - `imports` [EXTRACTED]
- [[recharts]] - `imports` [EXTRACTED]
- [[framer_motion]] - `imports` [EXTRACTED]
- [[leaflet]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Frontend